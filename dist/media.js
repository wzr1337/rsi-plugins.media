...
var Renderers = /** @class */ (function (_super) {
    ...
    function Renderers(service) {
        ...
        _this._renderers = [];
        _this._logger = core_1.rsiLogger.getInstance().getLogger("media.Renderers");
        ...
        var netfluxRenderer = new rxjs_1.BehaviorSubject({
            ...
            propertiesChanged: [],
            data: {
                uri: "/" + _this.service.name.toLowerCase() + "/" + _this.name.toLowerCase() + "/" + Renderers.netfluxRendererId,
        ...
        _this._renderers.push(netfluxRenderer);
...
var Collections = /** @class */ (function (_super) {
    ...
    function Collections(service) {
        ...
        _this.service = service;
        _this._collections = [];
        _this._logger = core_1.rsiLogger.getInstance().getLogger("media.Collections");
        var collectionId = "deadbeef-d2c1-11e6-9376-df943f51f0d8";
        var initialCollection = new rxjs_1.BehaviorSubject({
            lastUpdate: Date.now(),
            propertiesChanged: [],
            data: {
                uri: "/" + _this.service.name.toLowerCase() + "/" + _this.name.toLowerCase() + "/" + collectionId,
                id: collectionId,
                name: "default",
                items: []
            }
        });
        _this._collections.push(initialCollection);
        _this._change = new rxjs_1.BehaviorSubject({ lastUpdate: Date.now(), action: 'init' });
        _this._medialibrary = new medialibrary_1.Medialibrary();
        _this._tracks = _this._medialibrary.getResource("Tracks");
        return _this;
    }
